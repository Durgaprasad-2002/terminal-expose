...
// Close server and process on terminal exit
...
server.listen(PORT, HOST, async () => {
  const showToken = process.env.SESSION_TOKEN_REDACT !== "1";
  const displayToken = showToken ? SESSION_TOKEN : "***REDACTED***";

  const localUrl = `http://localhost:${PORT}/s/${displayToken}`;
  const externalUrl = EXTERNAL_URL
    ? `${EXTERNAL_URL.replace(/\/$/, "")}/s/${displayToken}`
    : null;
  ...
  getLanUrls({ port: PORT, session_token: displayToken }).forEach((url) =>
  ...
  if (options.publicTunnel) {
    try {
      ...
      const tunnelInfo = await startPublicTunnel(options, {
        HOST,
        PORT,
        SESSION_TOKEN,
      }).catch((error) => {
      ...
      if (tunnelInfo?.url) {
        ...
        terminalSession.readyForEcho = true;
      }
    } catch (error) {
      console.error(`Public tunnel failed: ${error.message}`);
    }
